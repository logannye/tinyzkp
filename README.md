...
- **Sublinear Space**: Proves traces of 4M rows using only ~2,048 row memory (2,000× less!)
- **Production Capacity**: 4M degree SRS (supports circuits up to 4,194,304 rows)
- **Production zkML**: Enable ResNet-34, BERT-base, GPT-2-medium inference proofs
...
**Traditional ZKP provers** require O(T) memory - proving a 4M row circuit needs 4M rows in memory (~128 MB).

**TinyZKP** uses streaming algorithms to prove with only O(√T) memory - proving a 4M row circuit needs just ~2,048 rows in memory (~32 KB).

**Result**: 
- 💾 **4,000× less memory** for large circuits (32 KB vs 128 MB)
- ⚡ **Faster proofs** on commodity hardware  
- 🧠 **zkML-ready** - ResNet-34, BERT-base, GPT-2-medium
...
| **Scale** | $199/mo | 250 | 4,194,304 rows (4M) | Enterprise zkML |
...
| **Scale (4M rows)** | 4,194,304 | **ResNet-34, BERT-base, GPT-2-medium** - production zkML |
...
- **Proof generation**: Up to 4M rows (Scale tier)

### 💳 Upgrading Your Account

1. Sign up for free tier (250 proofs/month, 32K rows)
2. Visit https://tinyzkp.com to upgrade
3. Choose Pro ($39/mo, 262K rows) or Scale ($199/mo, 4M rows)
...
Our production API uses a cryptographically-secure **4M degree SRS**:
- **Capacity**: Supports circuits up to **4,194,304 rows**
- **File size**: 128 MB (G1.bin) + 136 bytes (G2.bin)
- **Memory usage**: Only ~32 KB for 4M row proofs (O(√T) efficiency)
...
- **Enables**: Production zkML (ResNet-34, BERT-base, GPT-2-medium)
...
### SRS Usage
- **Production API**: Uses cryptographically-secure **4M degree SRS** (128 MB)
- **Capacity**: Up to **4,194,304 rows** per circuit (Scale tier)
- **Memory**: Only **~32 KB** for 4M row proofs (O(√T) advantage)
...
- Circuit size limits enforced per tier (Free: 32K, Pro: 262K, Scale: 4M)
